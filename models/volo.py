...
from prog.helpers import get_new_layer_idx
...
class Outlooker(nn.Module):
    ...
    def set_sample_config(self, is_identity_layer=False):
        self.is_identity_layer = is_identity_layer

    def forward(self, x):
        if not getattr(self, 'is_identity_layer', False):
...
class VOLO(nn.Module):
    ...
    def interpolate_pos_encoding(self, x):
        B, h0, w0, C = x.shape
        h, w = self.pos_embed.shape[1], self.pos_embed.shape[2]
        if h == h0 and w == w0:
    ...
    def set_sample_config(self, config: dict):
        l = config['layer_num']
        l_min = config['min_layer_num']
        l_max = config['max_layer_num']
        l0 = make_divisible(l * 0.23, 2)
        l0_min = make_divisible(l_min * 0.23, 2)
        l0_max = make_divisible(l_max * 0.23, 2)
        layers = [l0, l - l0, 0, 0]
        layers_min = [l0_min, l_min - l0_min, 0, 0]
        layers_max = [l0_max, l_max - l0_max, 0, 0]
        new_layer_idxs = [get_new_layer_idx(prev_l=layers_min[i], new_l=layers_max[i]) for i in range(4)]
        skip_layer_idxs = [new_layer_idxs[i] if (layers[i]-layers_min[i] == 0) else new_layer_idxs[i][:-(layers[i]-layers_min[i])] for i in range(4)]  # TODO: support more than 2x depth
        real_stage_idx = 0  # stage index that skips down-sampling layers
        for stage_idx in range(len(self.network)):
            if isinstance(self.network[stage_idx], (nn.Sequential, nn.ModuleList)):
                for layer_idx in range(len(self.network[stage_idx])):
                    self.network[stage_idx][layer_idx].set_sample_config(
                        is_identity_layer=layer_idx in skip_layer_idxs[real_stage_idx])
                real_stage_idx += 1
