...
from prog.helpers import load_checkpoint, resume_checkpoint
...
parser.add_argument('--batch-splits', type=int, default=1, metavar='N', help='steps interval to update network params,'
...
def main():
    ...
    if args.polyaxon:
        ...
        args.data_dir = os.path.join(get_data_paths()['host-path'], 'ImageNet')
        args.token_label_data = ''
        if args.token_label:
            args.token_label_data = os.path.join(get_data_paths()['host-path'], 'ImageNetTokenLabel/label_top5_train_nfnet')

        from polyaxon_client.tracking import get_outputs_path
        args.output = get_outputs_path()
    output_dir = ''
    ...
    assert args.batch_size % args.batch_splits == 0,\
        f'batch size {args.batch_size} should be divisible by batch splits {args.batch_splits}!'
    args.batch_size = args.batch_size // args.batch_splits
